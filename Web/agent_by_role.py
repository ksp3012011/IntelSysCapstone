...
path = "C:/Users/jyoung/Desktop/strategy/"
...
# 전략별 매매 판단 Agent
def strategy_agent(strategy_path, df_hourly, df_daily, orderbook):
    ...
    # 지표 계산
    df_hourly = calculate_indicators(df_hourly.copy(), strategy_path)
        ...
        messages=[
            {
                ...
                주어진 투자 전략을 기반으로 시간봉, 기술적 지표, 호가창을 분석.
                투자 전략은 시간봉에 적용하고, 일봉은 추세 파악에만 사용.
                판단 이유는 한글로 작성
        ...
        messages=[
            {
                ...
                뉴스 데이터와 공포 탐욕 지수를 분석해서 현재 시장이 어떤지 한글로 요약
...
def auto_trading():
    ...
    # 72시간 시간봉 데이터
    df_hourly = pyupbit.get_ohlcv("KRW-BTC", interval="minute60", count=72)
    df_hourly = dropna(df_hourly)

    # 전략별 매매 판단 받기
    agent_outputs = []
    for strategy_path in strategy_paths:
        decision = strategy_agent(strategy_path, df_hourly, df_daily, orderbook)
