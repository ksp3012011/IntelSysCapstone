...
from smtp import send_email, send_trade
import schedule
import time
...
def news():
    ...
    if news_object['status'] == 'ok':
        ...
        print("뉴스를 불러오는 데 문제가 발생했습니다. 상태 코드:", news_object['status'])
...
def save_trade(decision, reason):
    ...
    cursor.execute('''
    INSERT INTO trades (timestamp, symbol, decision, reason, btc_balance, krw_balance, btc_krw_price, avg_buy_price)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (timestamp, symbol, decision, reason, btc_balance, krw_balance, btc_krw_price, avg_buy_price))
...
# 전략별 매매 판단
def strategy_agent(strategy_path, df_5minute, df_hourly, orderbook):
    ...
    # 지표 계산
    df_5minute = calculate_indicators(df_5minute.copy(), strategy_path)
        ...
        messages=[
            {
                ...
                주어진 투자 전략을 기반으로 5분봉, 기술적 지표, 호가창을 분석.
                투자 전략은 5분봉에만 적용하고, 시간봉은 추세 파악에만 사용.
                판단 이유는 한글로 작성
...
# 실제 거래소 매매
def execute_trade(result, email):
    charge = 0.9995
    if result["decision"] == "buy":
        ...
        if buy_amount > 5000:   # 최소 주문 금액 5000krw
            ...
            #메일 전송      
            send_trade(email, "BTC", charge, buy_amount, "매수") #????????
        else:
            print("KRW 5000 미만")
            save_trade("buy", "최소 주문 금액보다 적어 매수가 불가합니다")
        ...
        if sell_amount * current_price > 5000:
            ...
            #메일 전송      
            send_trade(email, "BTC", sell_amount, sell_amount * current_price, "매도")
        else:
            print("BTC 5000 미만")
            save_trade("sell", "최소 주문 금액보다 적어 매도가 불가합니다")
...
def stop_auto_trading(profit):
    ...
    schedule.cancel_job(auto_trading)    # 자동매매 중단
    return
...
def auto_trading():
    ...
    for email in user_emails:
        ...
        # 72분 5분봉 데이터
        df_5minute = pyupbit.get_ohlcv("KRW-BTC", interval="minute5", count=72)
        df_5minute = dropna(df_5minute)

        # 전략별 매매 판단 받기
        agent_outputs = []
        for strategy_path in strategy_paths:
            decision = strategy_agent(strategy_path, df_5minute, df_hourly, orderbook)
        ...
        # 매매 실행
        execute_trade(final_decision, email)
...
if __name__ == "__main__":
    ...
    """
    # 15분마다 자동매매 진행
    schedule.every(5).minutes.do(auto_trading)
